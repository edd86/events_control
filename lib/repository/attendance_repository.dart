...
class AttendanceRepository {
  ...
  Future<List<Attendance>> getAttendances() async {
    ...
    attendanceMaps.map((attendanceMap) => attendances.add(Attendance.fromMap(attendanceMap)));
    return attendances;
  }
  ...
  Future<Attendance> updateAttendance(Attendance attendance) async {
    final db = await DatabaseHelper().database;
    await db
        .update('attendances', attendance.toMap(), where: 'id = ?', whereArgs: [attendance.id]);
    return attendance;
  }

  Future<Attendance> deleteAttendance(Attendance attendance) async {
    final db = await DatabaseHelper().database;
    await db.delete('attendances', where: 'id = ?', whereArgs: [attendance.id]);
    return attendance;
  }
}
